...
contract Dice2Win {
    ...
    //  - 2 for coin flip 抛硬币
    //  - 6 for dice   骰子
    //  - 6*6 = 36 for double dice  两个骰子
    //  - 100 for etheroll 过山车
    //  - 37 for roulette  轮盘堵
    //  etc.
    ...
    struct Bet {
        // Wager amount in wei. 下注金额
        uint amount;
        // Modulo of a game. 游戏模数
        uint8 modulo;
        // Number of winning outcomes, used to compute winning payment (* modulo/rollUnder),
        // and used instead of mask for games with modulo > MAX_MASK_MODULO.
        // 获胜的数量, 用于计算奖金
        uint8 rollUnder;
        // Block number of placeBet tx.  下注交易所在的区块
        uint40 placeBlockNumber;
        // Bit mask representing winning bet outcomes (see MAX_MASK_MODULO comment). 掩码
        uint40 mask;
        // Address of a gambler, used to pay out winning bets. 赌徒的地址
        address gambler;
    }
    ...
    // Change the croupier address. 设置荷官的地址
    function setCroupier(address newCroupier) external onlyOwner {
        croupier = newCroupier;
    }

    // Change max bet reward. Setting this to zero effectively disables betting.
    // 最高投注奖励, 可以设置为0,以禁止玩家投注
    ...
    //                    after placeBet have been mined.  用了 "秘密-揭示"技术, 用于开注
    ...
    function settleBetCommon(Bet storage bet, uint reveal, bytes32 entropyBlockHash) private {
        ...
        bytes32 entropy = keccak256(reveal, entropyBlockHash);
        ...
        // Log jackpot win.
        if (jackpotWin > 0) {
            JackpotPayment(gambler, jackpotWin);
        }
    ...
    function getDiceWinAmount(uint amount, uint modulo, uint rollUnder) private pure returns (uint winAmount, uint jackpotFee) {
        ...
        require (houseEdge + jackpotFee <= amount);
